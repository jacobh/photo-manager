...
describe("main window", function spec() {
  ...
  const findCounter = () =>
    this.app.client.element(`.${counterStyles.counter}`);
  ...
  it("should back to home if back button clicked", async () => {
    const { client } = this.app;
    await client.element(`.${counterStyles.backButton} > a`).click();

    expect(await client.isExisting(`.${homeStyles.container}`)).to.be.true;
  });
});
